...
{
	PrintToChat(client, "Your \x03gold knife\x01 has been disabled. Type '!goldknifeme' to get it back.");
	KnifeEnabled[client] = 1;
}

public Action:Command_goldknifeme(client, args)
{
	PrintToChat(client, "Your \x03gold knife\x01 has been enabled. Type '!nogoldknife' to disable it.");
	KnifeEnabled[client] = 0;
...
public Event_PlayerSpawn(Handle:event, const String:name[], bool:dontBroadcast){
	...
	if (GetConVarInt(g_Cvar_Knife_Enabled) > 0 && GetAdminFlag(GetUserAdmin(client), Admin_Reservation)) 
	{
		if(GetClientTeam(client) == 2 || GetClientTeam(client) == 3)
		{
			...
			{
				PrintToChat(client, "\x01Type '!goldknifeme' to enable your \x03gold knife");				
			}
		}
	}
