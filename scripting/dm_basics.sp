...
{
	new client = GetClientOfUserId(GetEventInt(event, "userid"));
	if (GetClientCount() < 13)
	{
		CreateTimer(0.5, QuickRespawn, client, TIMER_FLAG_NO_MAPCHANGE);
	}
	else if(GetClientCount() < 17)
	{
		CreateTimer(1.5, QuickRespawn, client, TIMER_FLAG_NO_MAPCHANGE);		
	}
	else if(GetClientCount() < 19)
	{
		CreateTimer(2.0, QuickRespawn, client, TIMER_FLAG_NO_MAPCHANGE);		
	}
}

public Action:QuickRespawn(Handle:timer, any:client)
{
...
public Action:Timer_Welcome(Handle:timer, any:userid)
{
	...
	PrintToChat(client, "[-abM- DM] Welcome to the -abM- FFA DM server.");
	PrintToChat(client, "[-abM- DM] Visit http://abm.site.nfoservers.com to support the server or give feedback.");
...
public Action:Command_Say(client, args)
{
	...
	if (strcmp(command, "ssrtnka") == 0)
	{

		/* There are no tests... respawn! */
		PrintToChat(client, "How did you find that?");
		DM_RespawnClient(client);
