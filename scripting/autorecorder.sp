...
public StartRecord()
{
	...
	{
		decl String:sPath[PLATFORM_MAX_PATH], String:sTime[16], String:sMap[64], String:part[32];
		
		GetConVarString(g_hDemoPath, sPath, sizeof(sPath));
		FormatTime(sTime, sizeof(sTime), "%Y%m%d-%H%M%S", GetTime());
		GetCurrentMap(sMap, sizeof(sMap));
		if(strncmp(sMap, "workshop/", 9) == 0)
		{
			ReplaceString(sMap, sizeof(sMap), "workshop/", "", false);
			SplitString(sMap, "/", part, sizeof(part));
			ReplaceString(sMap, sizeof(sMap), part, "", false);
			ReplaceString(sMap, sizeof(sMap), "/", "", false);
		}
		ServerCommand("tv_record \"auto-%s-%s\"", sTime, sMap);
		g_bIsRecording = true;
		
		PrintToChatAll("GOTV: Recording to auto-%s-%s.dem", sTime, sMap);
...
public StopRecord()
{
	...
	{
		PrintToChatAll("GOTV: Stopped Recording");
