 ...
 *
 *	CHANGES BY KHAN
 *	- Plugin will now disarm first and slay after 5 seconds (Breaks a lot of the other stuff but we don't use that shit anyway)
 *	- Removed #include <colors> and replaced all CPrintToChat with PrintToChatAll (<colors> doesn't even work with CS:GO so fuck it)
 *
 *
 */
...
public Action:EventRoundEnd(Handle:event, const String:name[], bool:dontBroadcast)
{
  ...
  if (g_bCvarEnabled && g_bCvarSlayOnRoundTimeUp)
  {
    ...
    if (winner == CS_TEAM_CT || winner == CS_TEAM_T)
    {
      ...
      {
        slay = false;
        PrintToChatAll("Counter Terrorists have been spared for touching at least one hostage");
      }
      if (slay)
      {
...
public Action:EventBombDefused(Handle:event, const String:name[], bool:dontBroadcast)
{
  if (g_bCvarEnabled && g_bCvarSlayOnObjectives)
  {
    ...
    {
      PrintToChatAll("Terrorists have been spared for wire used");
    }
    else
    {
...
public Action:EventBombExploded(Handle:event, const String:name[], bool:dontBroadcast)
{
  if (g_bCvarEnabled && g_bCvarSlayOnObjectives)
  {
    if (g_bCvarDNSIfQuickDefuseWireUsed && g_bCvarQuickDefuseWireUsed)
    {
      PrintToChatAll("Counter Terrorists have been spared for wire used");
    }
    else
    {
...
public Action:SlayTeam(Handle:t, any:team)
{
  if(bCanSlay) //avoid slaying right after round starts, if bomb explodes 0.1 seconds before a new round
  {
    ...
    if (slayedcount > 0)
    {
      if (team == CS_TEAM_CT)
      {
          PrintToChatAll("Counter Terrorists have been disarmed for not completing the objectives");
      }
      else if (team == CS_TEAM_T)
      {
          PrintToChatAll("Terrorists have been disarmed for not completing the objectives");
      }
    }
	CreateTimer(5.0, SlayTeam2, team);
  }
}

public Action:SlayTeam2(Handle:t, any:team)
{
  if(bCanSlay) //avoid slaying right after round starts, if bomb explodes 0.1 seconds before a new round
  {
    new slayedcount = 0;
    ...
    if (slayedcount > 0)
    {
      if (team == CS_TEAM_CT)
      {
          PrintToChatAll("Counter Terrorists have been slayed for not completing the objectives");
      }
      else if (team == CS_TEAM_T)
      {
          PrintToChatAll("Terrorists have been slayed for not completing the objectives");
      }
    }
  }
}
